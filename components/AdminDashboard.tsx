...
function exportCSV(reservations: Reservation[]) {
  const headers = ['ID', 'Date', 'Nom', 'Email', 'Téléphone', 'Formule', 'Pack', 'Début', 'Fin', 'Total TTC', 'Acompte', 'Statut', 'Notes']
    ...
    <div className="res-card">
      ...
      <div className="res-details">
        ...
        <div>
          ...
          <div className="res-detail-value">
            {r.dateDebut}{r.dateFin ? ` → ${r.dateFin}` : ''}
            {r.heureDebut ? ` · ${r.heureDebut}–${r.heureFin}` : ''}
          </div>
        </div>
        <div>
          <div className="res-detail-label">Statut pro</div>
          <div className="res-detail-value capitalize">{r.statutPro.replace('-', ' ')}</div>
        </div>
        <div>
      ...
      <div className="res-pricing">
        ...
        <div className="res-meta">
          Réf. {r.id}<br />{formatDate(r.createdAt)}
        </div>
      </div>
      ...
      <div className="res-actions">
        {r.status !== 'confirmed' && (
          <button className="btn btn-success btn-sm" onClick={() => onStatusChange(r.id, 'confirmed')}>✓ Confirmer</button>
        )}
        {r.status !== 'cancelled' && (
          <button className="btn btn-danger btn-sm" onClick={() => onStatusChange(r.id, 'cancelled')}>✕ Annuler</button>
        )}
        {r.status === 'cancelled' && (
          <button className="btn btn-ghost btn-sm" onClick={() => onStatusChange(r.id, 'pending')}>↩ Remettre en attente</button>
        )}
        <a href={whatsappLink(r.telephone, waMsg)} target="_blank" rel="noopener noreferrer" className="btn btn-wa btn-sm">
          💬 WhatsApp
        </a>
      </div>
    </div>
        ...
        <div className="login-card">
          ...
          <form onSubmit={login}>
            <div className="form-group">
              ...
              {pwdError && <p className="form-error">⚠ Mot de passe incorrect</p>}
            </div>
            <button type="submit" className="btn btn-primary btn-full login-submit">
  ...
  return (
    <main>
      <div className={`toast${toast ? ` show ${toast.type}` : ''}`}>
        <div className="toast-title">{toast?.type === 'success' ? '✓' : '⚠'} {toast?.msg}</div>
      </div>

      <div className="admin-wrap">
        <div className="admin-header">
          <div>
            <Link href="/" className="back-link admin-breadcrumb">← Retour au site</Link>
        ...
        <div className="filter-bar">
          ...
          <select
            className="form-select filter-select"
            value={filterFormule}
            onChange={e => setFilterFormule(e.target.value as FormulaType | 'all')}
          >
